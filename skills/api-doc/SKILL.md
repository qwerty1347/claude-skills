...
## 5. 이전 문서를 찾아 설명만 이어받는다

출력 폴더를 훑어 같은 API 의 지난 명세서가 있는지 본다.

```
Glob: {출력 폴더}/*.html
```

**파일명만 보고 판단하지 않는다.** 같은 API 라도 지난번엔 `사용자목록`, 이번엔 `회원조회`
로 이름을 지었을 수 있다. 이름이 비슷한 후보를 열어 문서 안의 `METHOD /경로` 줄이
지금 대상과 같은지로 확인한다. 경로가 API 의 신원이고 주제 이름은 흔들린다.

여러 개가 걸리면 **파일명 날짜가 가장 최근인 것**을 쓴다.

### 찾았으면

**읽기만 한다. 그 파일을 고치거나 지우지 않는다.**

가져오는 것은 하나다. **손으로 적어둔 설명을 그대로 옮겨온다.** 코드에서 나오지 않는
문장이라 다시 못 만든다. 지금도 코드에 있는 파라미터의 설명만 옮긴다.
주제 이름도 이전 문서 것을 그대로 써서 다음에도 같은 이름으로 묶이게 한다.

옮겨온 뒤에도 **명세서 내용은 현재 코드 그대로다. 무엇이 바뀌었는지는 기록하지 않는다.**
...
그냥 새로 만든다. 찾는 데 시간을 더 쓰지 않는다.

## 6. HTML 을 만든다

1단계에서 받은 폴더에 `주제_YYYY-MM-DD.html` 로 **항상 새로 만든다.**

주제를 앞에 두는 이유는 같은 API 의 문서가 이름순으로 나란히 붙게 하기 위한 것이다.
5단계에서 이전 문서를 찾았어도 **그 파일을 고치지 않는다.** 내용을 잘못 건드려
멀쩡하던 명세서를 망치는 것보다, 파일이 하나 더 쌓이는 편이 낫다.

- 날짜는 만드는 날 기준
- 같은 날 같은 주제를 다시 만들면 그 파일만 덮어쓴다. 오늘 것이라 잃을 게 없다
...
`assets/doc.css` 의 클래스를 쓴다. **배지는 전부 `<span>` 이다.**
...
배지를 `<code>` 로 감싸지 않는다. 지라가 칩 배경을 그려주는 대신 글자색을 버려서
타입이 전부 같은 회색이 된다.

**색을 즉석에서 정하지 않는다.** 지라는 붙여넣은 배경을 전부 버리고 글자색만 남기는데,
그 글자색도 자기 팔레트에 있는 값만 남기고 나머지는 조용히 검정으로 바꾼다.
쓸 수 있는 색은 `assets/doc.css` 주석에 적힌 14색뿐이다. 새 배지가 필요하면
그 목록에서 고른다.
...
- 이전 문서를 고치지 않는다. 읽어서 설명만 가져오고 문서는 항상 새로 만든다
...
- 지난 문서를 지우지 않는다. 오래됐어도 그대로 둔다. 정리는 사람이 판단한다
- 배지를 `<code>` 로 만들지 않는다. 지라가 글자색을 버려서 타입 구분이 사라진다
